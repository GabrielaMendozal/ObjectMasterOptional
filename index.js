const pokemon = Object.freeze([
    ...
    //lista de pokémon que tengan nombres que comiencen con la letra "B" usando el siguiente código.
    ...
    const divisibles3 = pokemon.filter( p => p.id % 3 === 0);
    console.log( divisibles3);

    // una serie de objetos Pokémon que son del tipo "fuego"
    const typeFuego = pokemon.filter( p => p.types[0] === "fire");
    console.log( typeFuego);

    //Una variedad de objetos Pokémon que tienen más de un tipo
    const typesVarios = pokemon.filter( p => p.types.length > 1);
    console.log( typesVarios);
    ...
    const typeVeneno = pokemon.filter( p => p.types.length === 1 && p.types.find( type => type.includes("poison") )).map(p => p.name);
    console.log( typeVeneno);

    //una matriz que contiene solo el primer tipo de todos los Pokémon cuyo segundo tipo es "volador"
    const typeVolador = pokemon.filter(p => p.types.length > 1 && p.types[1].includes("flying")).map(p =>p.types[0]);
    console.log( typeVolador);

    //un recuento de la cantidad de pokémon que son de tipo "normal"
    const pokemonNormal =pokemon.filter( p => p.types.find( type => type.includes ("normal"))).length;
